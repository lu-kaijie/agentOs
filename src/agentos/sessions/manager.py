...
class SessionManager:
    ...
    def build_resume_state(self, session_id: str) -> tuple[dict[str, object], str]:
        ...
        carryover = {
            ...
            "execution_mode": str(state.get("execution_mode", "deterministic")),
            "background_results": list(state.get("background_results", [])),
            "consumed_background_jobs": list(state.get("consumed_background_jobs", [])),
            "next_pending_tasks": list(state.get("next_pending_tasks", [])),
            "approval_policy": dict(state.get("approval_policy", {})),
            "pending_approval": dict(state.get("pending_approval", {})),
            "approval_response": "",
            "approval_outcome": dict(state.get("approval_outcome", {})),
            ...
            "context_audit_records": list(state.get("context_audit_records", [])),
        }
