...
from agentos.agents.parser import AgentDef, AgentParseError, parse_agent_file, parse_frontmatter
...
from agentos.agents.fork import (
    FORK_BOILERPLATE_TAG,
    ForkError,
    build_forked_messages,
)
from agentos.agents.trace import TraceManager, TraceNode
from agentos.agents.task_manager import BackgroundTask, TaskManager
from agentos.agents.notification import format_task_notification, inject_task_notifications
from agentos.conversation import ConversationManager, Message, ToolResultBlock, ToolUseBlock
...
class TestAgentLoader:
    ...
    def test_project_overrides_builtin(self, tmp_path: Path):
        agents_dir = tmp_path / ".agentos" / "agents"
...
class TestToolFilter:
    ...
    def test_builtin_no_custom_restrictions(self):
        # EnterPlanMode 现在已归入 ALL_AGENT_DISALLOWED，所以应当用一个
        # 只在 CUSTOM 而不在 ALL 中的工具，来验证内置 agent 会跳过
        # custom 这一层。这里只验证内置 agent 仍然能拿到正常的工具。
...
class TestPermissionMode:
    def test_dont_ask_mode(self):
        from agentos.permissions.modes import PermissionMode, mode_decide
...
class TestAgentToolParams:
    def test_required_fields(self):
        from agentos.tools.agent_tool import AgentToolParams
