...
from agentos.hooks import HookConfigError, HookEngine, load_hooks
from agentos.permissions import PermissionMode


def main() -> None:
    # 先确保 .agentos/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".agentos").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".agentos/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="agentos", description="AgentOS AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    from agentos.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str) -> None:
    ...
    from agentos.client import create_client, resolve_context_window
    from agentos.conversation import ConversationManager
    from agentos.memory.instructions import load_instructions
    ...
    from agentos.tools import create_default_registry
