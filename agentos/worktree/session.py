...
def _session_path(agentos_dir: Path) -> Path:
    return agentos_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(agentos_dir)
...
def load_worktree_session(agentos_dir: Path) -> WorktreeSession | None:
    path = _session_path(agentos_dir)
    if not path.exists():
        return None
    try:
