...
from agentos.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from agentos.conversation import ThinkingBlock as ConvThinkingBlock
from agentos.memory.auto_memory import MemoryManager
from agentos.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from agentos.hooks import HookContext, HookEngine, ToolRejectedError
from agentos.hooks.engine import HookNotification
from agentos.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".agentos" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".agentos" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from agentos.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from agentos.context.manager import (
