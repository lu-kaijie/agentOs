...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".agentos" / "config.yaml",
        cwd / ".agentos" / "config.yaml",
        cwd / ".agentos" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .agentos/config.yaml "
            "in project or ~/.agentos/config.yaml"
        )
    return merged
