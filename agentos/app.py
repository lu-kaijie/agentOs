...
from agentos.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from agentos.commands.completion import CompletionPopup
from agentos.commands.handlers import register_all_commands
from agentos.config import MCPServerConfig, ProviderConfig
from agentos.hooks import HookContext, HookEngine, load_hooks
from agentos.conversation import ConversationManager, Message
from agentos.mcp import MCPManager
...
from agentos.agents.task_manager import TaskManager
from agentos.agents.trace import TraceManager
from agentos.agents.notification import inject_task_notifications
from agentos.commands.handlers.tasks import create_tasks_command
from agentos.skills.executor import SkillExecutor
from agentos.skills.loader import SkillLoader
from agentos.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from agentos.cache import FileCache
from agentos.tools import ToolRegistry, create_default_registry
from agentos.tools.agent_tool import AgentTool
from agentos.tools.ask_user import AskUserEvent, AskUserTool
from agentos.tools.impl.tool_search import ToolSearchTool
from agentos.tools.load_skill import LoadSkill
from agentos.worktree.cleanup import start_stale_cleanup_task
from agentos.worktree.manager import WorktreeManager
from agentos.commands.handlers.worktree import create_worktree_command
from agentos.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".agentos", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".agentos" / "history"
...
class AgentOSApp(App):
    ...
    def on_mount(self) -> None:
        self.register_theme(_AGENTOS_THEME)
        self.theme = "agentos"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".agentos" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".agentos" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".agentos" / "permissions.local.yaml",
            ),
        ...
        from agentos.filehistory import FileHistory
        ...
        from agentos.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from agentos.config import WorktreeConfig
        ...
        from agentos.tools.enter_worktree import EnterWorktreeTool
        from agentos.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from agentos.teams.manager import TeamManager
        from agentos.tools.team_create import TeamCreateTool
        from agentos.tools.team_delete import TeamDeleteTool
        ...
        from agentos.commands.handlers.trace import create_trace_command
        ...
        from agentos.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from agentos.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from agentos.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from agentos.plan_dialog import InlinePlanWidget, PlanChoice
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from agentos.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from agentos.permission_dialog import InlinePermissionWidget
